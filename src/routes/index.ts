...
import {get, getSimilar, list, random, trim} from "@services/parser";
...
router.get('/group/:weighted?/:max?/:page?',
    ...
    async (req: Request, res: Response) => {
        ...
        if (image) res.send(trim(await getSimilar(image), props.max, props.page));
...
router.get(
    ...
    async (req: Request, res: Response) => {
        ...
        if (image) res.send(trim(await getSimilar(image), params.max, params.page));
        else res.sendStatus(404);
    }
);
